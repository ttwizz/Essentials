...
local Gui = Player.PlayerGui:FindFirstChild('BAE3.0 Client')

repeat
    Gui = Player.PlayerGui:FindFirstChild('BAE3.0 Client')
	if Gui then
		break
	end
	wait()
...
local essentialsFolder = replicatedStorage:WaitForChild('英雄冒着生命危险去拯救别')
local essentialsEvent = essentialsFolder:WaitForChild(string.reverse("险去拯" .. tostring(game.PlaceId * 7.81294518743) .. "雄冒着"))
local essentialsFunction = essentialsFolder:WaitForChild(string.reverse("生命" .. tostring(game.PlaceId * 4.79019846253) .. "险去拯救"))
local nowTime = tick()
...
local function fireServer(...)
	if integrityCheck(1) then
        ...
        essentialsEvent:FireServer(clientConfig.Key,string.reverse("\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r")..string.reverse(string.lower("\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r"))..string.reverse(string.upper("\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r")).."\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r"..string.lower("\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r")..string.upper("\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r"),...)
	end
end
...
local function Display(Type,Data)
	...
	elseif Type == "List" then
		...
		table.insert(Connections,exitButton.MouseButton1Click:connect(function()
			spawn(function()
				for a,b in next, scrollingFrame:GetChildren() do
			...
			for a,b in next, Stacks.Frames do
				if b == listClone then
...
--				searchBar:TweenPosition(UDim2.new(0,0,1,0),'Out','Quint',0.3,true)
	...
	elseif Type == "Donor" then
		...
		spawn(function()
			local nDown = 0
			for i,v in next, materialList do
	...
	elseif Type == "PBans" then
		...
		function loopManager:NewLoop(Table)
			for a,b in next, mainScroll:GetChildren() do
				if b.Name ~= 'Template' then
					b:Destroy()
				end
			end
					...
					table.insert(Connections,Obj.MouseEnter:connect(function()
						table.insert(Connections,Obj.MouseMoved:connect(function(X,Y)
							if Obj.Text == "This ban is a legacy ban." then
                                mouseLabel.Text = "Essentials 3.0 did some data restructuring, and this ban was from the old data structure."
							else
...
local function pendNotif(Title,Desc,Data)
	spawn(function()
		...
		exitConnection = notifExit.MouseButton1Click:connect(function()
			...
			for a,b in next, Stacks.Notifs do
...
spawn(function()
	...
	runService.RenderStepped:connect(function()
		for a,b in next,Players:GetPlayers() do
            if b.Character and b.Character:FindFirstChild('BAE3.0 Cape') and b.Character:FindFirstChild('Humanoid') then
                local Cape = b.Character['BAE3.0 Cape']
...
local function Console()
	pcall(function()
		...
		if not consoleOpen then
			...
			spawn(function()
				for i,v in next,baseClip:GetChildren() do
...
function essentialsFunction.OnClientInvoke(Starter,...)
	...
	elseif Starter == "Client Setup" then
		if not clientConfig.Permission and not clientConfig.Key then
            ...
            testService:Message("Essentials 3.0 | "..clientConfig.Version.." | Prefix: \""..clientConfig.Prefix.."\" | Act. Prefix: \""..clientConfig.actionPrefix.."\"")
			...
			elseif clientConfig.Permission == 4 then
                ...
                testService:Message("Essentials 3.0 | DP: "..tostring(clientConfig.donorEnabled).." | CD: "..tostring(clientConfig.Debugging))
			end
			if adminTitle then
...
local function localName(otherPlayer,Data)
	if otherPlayer and otherPlayer.Character then
        local oldNamed = otherPlayer.Character:FindFirstChild('BAE3.0_Named',true)	
		...
		if Data then
            ...
            removeOldTag.Name = "BAE3.0_Named"
