--[[
	...
	created by ttwiz_z#2081, otherwise it was created by their respective
	owners.
...
local clientCode = Components:WaitForChild('BAE3.0 Code')
local essentialsUI = Components:WaitForChild('BAE3.0 Client')
...
local F3X = Components:WaitForChild('Building Tools')
...
local sysTable = {
    ...
    exploitMessage = "An error occurred.",
...
If you have any questions or concerns, feel free to PM ttwiz_z#2081 on Discord.]]
}
...
function Funcs.Display(Args)
    ...
    elseif Command == "changelog" or Command == "clog" then
		essentialsEvent:FireClient(Player,'PM',"Changelog",sysTable.Changelog,true)
    elseif Command == "trellobans" or Command == "tbans" then
    ...
    elseif Command == "ingameadmins" or Command == "igadmins" then
		local Table = {}
    ...
    elseif Command == "chatlogs" or Command == "clogs" then
	...
	elseif Command == "logs" then
		...
		if newLogTable then
			local PSA = returnPSAMessage(Player,'all') or returnPSAMessage(Player,'logs')
			essentialsEvent:FireClient(Player,'List','Command Logs',true,true,newLogTable,PSA)
		end
    elseif Command == "joinlogs" or Command == "jlogs" then
		essentialsEvent:FireClient(Player,'List','Join Logs',true,true,sysTable.joinLogs)
--	elseif Command == "pbans" or Command == "permbans" then
...
--		end
    ...
    elseif Command == "leavelogs" or Command == "llogs" then
        essentialsEvent:FireClient(Player,'List','Leave Logs',true,true,sysTable.leaveLogs)
    elseif Command == "shutdownlogs" or Command == "sdlogs" then
    ...
    elseif Command == "debuglogs" or Command == "dlogs" then
...
function Funcs.displayMessage(Args)
	...
	local Command = Args[2]
	local combinedArgs = ""
    if Command == "smtest" or Command == "sysmsgtest" then
    else
		if not Args[3] then return end
		for a,b in next, (Args) do
			if a > 2 then
    ...
    if Command == "sm" or Command == "servmsg" then
		if combinedArgs ~= "" then
			for a,b in next,playerService:GetPlayers() do
				local cleansedData = ""
    ...
    elseif Command == "countdown" or Command == "cd" then
...
--								essentialsEvent:FireAllClients('Hint','Countdown',"BEGIN!")
    ...
    elseif Command == "m" or Command == "message" then
		if combinedArgs ~= "" then
			for a,b in next,playerService:GetPlayers() do
				local cleansedData = ""
				local cannotShow = false
    ...
    elseif Command == "h" or Command == "hint" then
		if combinedArgs ~= "" then
			for a,b in next,playerService:GetPlayers() do
				local cleansedData = ""
				local cannotShow = false
	...
	elseif Command == "smtest" or Command == "sysmsgtest" then
        essentialsEvent:FireAllClients('Message',"Server Message",[[Героический поступок

Героический поступок – это поступок, который совершает человек в трудных и опасных условиях. При этом человек рискует своей жизнью, спасая других. 

Много было совершено героических подвигов людьми во время Великой Отечественной войны. Об одном из них я хочу рассказать.

Одним из участников Великой Отечественной войны был Осипенко Ефим Ильич. Он был минёром и командиром партизанского отряда. Чтобы остановить подход немецкой армии к Москве, партизаны решили взорвать важный участок железной дороги у станции «Мышбор». Осипенко Е.И. отправил друзей в безопасное место и остался один. После того как взрывчатка не сработала, партизан ударил по бомбе шестом от железнодорожного знака. В результате был пущен под откос вражеский состав с танками. Партизан был тяжело ранен и потерял зрение.

Осипенко Е.И. совершил настоящий подвиг. Он проявил храбрость, отвагу и способность к самопожертвованию. Ведь не каждый человек готов рисковать собой, чтобы совершить что-то на благо других.]])
	end
end
...
function Funcs.Admin(Args)
	...
	local Victims = returnPlayers(Player,Args[3],Args[2])
	if not Victims then return end
    if Command == "superadmin" or Command == "supadmin" then
		for a,b in next,Victims do
			if b ~= Player then
    ...
    elseif Command == "mod" or Command == "moder" then
		for a,b in next,Victims do
			if b ~= Player then
				local Permission = returnPermission(b)
    ...
    elseif Command == "unadmin" or Command == "unmoder" or Command == "unmod" or Command == "unsupadmin" or Command == "unsuperadmin" then
		for a,b in next,Victims do
			local Permission = returnPermission(b)
			for c,d in next,sysTable.Permissions do
...
function Funcs.lockSever(Args)
    ...
    elseif Cmd == "unslock" or Cmd == "unserverlock" then
...
function Funcs.Teleport(Args)
	local Player = Args[1]
    if Args[2] == 'tp' or Args[2] == 'teleport' then
		...
		for a,b in next, (Players) do
			...
			if bHumanoid ~= nil and bHumanoidRootPart ~= nil and tHumanoidRootPart ~= nil then
				if bHumanoid.SeatPart ~= nil then
                    local AlreadyUnSitting = bHumanoid:FindFirstChild("BAE3.0_Unsitting")
					if not AlreadyUnSitting then
						AlreadyUnSitting = Instance.new("Folder")
                        AlreadyUnSitting.Name = "BAE3.0_Unsitting"
						...
						spawn(function()
							...
							if (bHumanoid.Parent.Parent ~= nil) and (bHumanoid:IsDescendantOf(b.Character)) then
								for c,d in next,bHumanoid:GetChildren() do
                                    if d.Name == "BAE3.0_Unsitting" then
										d:Destroy()
									end
								end
			...
			for a,b in next, (Players) do
				...
				if bHumanoidRootPart ~= nil then
					if playerHumanoid.SeatPart ~= nil then
                        local AlreadyUnSitting = playerHumanoid:FindFirstChild("BAE3.0_Unsitting")
						if not AlreadyUnSitting then
							AlreadyUnSitting = Instance.new("Folder")
                            AlreadyUnSitting.Name = "BAE3.0_Unsitting"
...
function Funcs.Info(Args)
	...
	for a,b in next, (Players) do
        ...
        table.insert(InfoTable,"Essentials 3.0 Donor: "..Donor)
		local Permission = returnPermission(b)
	...
	for a,b in next, (Players) do
		if checkAsset(Args[4],8) or checkAsset(Args[4],41) or checkAsset(Args[4],42) or checkAsset(Args[4],43) or checkAsset(Args[4],44) or checkAsset(Args[4],45) or checkAsset(Args[4], 46) then
			...
			if insertedItem:IsA('Accoutrement') then
				for c,d in next, (insertedItem:GetDescendants()) do
...
local function banId(playerId,playerName,Reason)
    ...
    pluginEvent:Fire("Exploit Logs",{'[EXPLOITER BANNED] '..playerId..', '..playerName})
    addLog(sysTable.debugLogs,'[EXPLOITER BANNED] '..playerId..', '..playerName)
end
...
function Funcs.permBan(Args)
	...
	local Cmd = Args[2]
	local Players = returnPlayers(Player,Args[3],Args[2])
    if Cmd == "pban" or Cmd == "pbanid" or Cmd == "permban" or Cmd == "permbanid" then
		local Succ,Msg = pcall(function()
			local victimTable = {}
			...
			if not Players then
				local Succ,Msg = pcall(function()
                    if Cmd == "pban" or Cmd == "permban" then
		...
		if Msg then
            addLog(sysTable.debugLogs,'Funcs.permBan, "pban (permban)", Message: '..Msg or "Err")
		end
    elseif Cmd == "unpban" or Cmd == "unpbanid" or Cmd == "unpermban" or Cmd == "unpermbanid" then
		local Succ,Msg = pcall(function()
			local victimId,victimName,Removed
			local Succ,Msg = pcall(function()
                if Cmd == "unpban" or Cmd == "unpermban" then
					victimId = tonumber(playerService:GetUserIdFromNameAsync(Args[3]))
					victimName = tostring(playerService:GetNameFromUserIdAsync(victimId))
				else
					victimId = tonumber(Args[3])
			...
			if Succ and not Msg then
				...
				if Succ and bannedStatus then
					...
					if Succ then
						Removed = true
					elseif Msg then
                        addLog(sysTable.debugLogs,'Funcs.permBan remove, "unpban (unpermban)", Message: '..Msg or "Err")
					end
				elseif Succ and not bannedStatus then
					...
					if Msg then
                        addLog(sysTable.debugLogs,'Funcs.permBan update, "unpban (unpermban)", Message: '..Msg or "Err")
					end
				end
			else
				essentialsEvent:FireClient(Player,'Message','Error','An error occurred while trying to "'..Cmd..'"'..'\n'..Msg)
		...
		if Msg then
            addLog(sysTable.debugLogs,'Funcs.permBan, "unpban (unpermban)", Message: '..Msg or "Err")
		end
    elseif Cmd == "pbans" or Cmd == "permbans" then
	...
	if Args[4] and Args[5] then
		...
		for a,b in next, (Players) do
			if b:FindFirstChild('leaderstats') then
				...
				for c,d in next, (leaderStats:children()) do
					if d.Name:lower():find(Args[4]:lower()) == 1 then
						d.Value = cleansedVal
					end
				end
...
function Funcs.Utility(Args)
    ...
    if Args[2] == 'btools' or Args[2] == 'f3x' then
		for a,b in next, (Players) do
			local F3XClone = F3X:Clone()
			F3XClone.Parent = b.Backpack
		end
    elseif Args[2] == 'segway' or Args[2] == 'hs' then
		for a,b in next, (Players) do
...
function Funcs.Give(Args)
	...
	local Players = returnPlayers(Player,Args[3],Args[2])
	for a,b in next, (Players) do
		for c,d in next, (sysTable.toolLocation:GetDescendants()) do
			if (d:IsA('Tool') or d:IsA('HopperBin')) and not playerService:GetPlayerFromCharacter(d.Parent) then
				if Args[4]:lower() ~= 'all' then
					if d.Name:lower():find(Args[4]:lower()) == 1 then
                        ...
                        if Args[2] == 'startergear' or Args[2] == 'sg' then
...
local function awardCape(Player,cColor,cMat,cDecal,cTrans)
	...
	local cDecal = cDecal
    if b.Character and b.Character:FindFirstChild('Humanoid') and not b.Character:FindFirstChild('BAE3.0 Cape') then
    ...
    elseif b.Character and b.Character:FindFirstChild('Humanoid') and b.Character:FindFirstChild('BAE3.0 Cape') then
        b.Character['BAE3.0 Cape']:Destroy()
...
function Funcs.Cape(Args)
	local Player = Args[1]
	local Players = returnPlayers(Player,Args[3],Args[2]) if not Players then return end
    if Args[2] == "awardcape" or Args[2] == "awcape" then
		for a,b in next, (Players) do
			awardCape(b)
		end
	elseif Args[2] == 'uncape' then
		for a,b in next,Players do
            if b.Character and b.Character:FindFirstChild('BAE3.0 Cape') then
                b.Character['BAE3.0 Cape']:Destroy()
			end
		end
	end
...
function Funcs.itemStats(Args)
	local Player = Args[1]
	local Command = Args[2]
    local InfoType = ((Command == "gamepassinfo" or Command == "gpinfo") and Enum.InfoType.GamePass) or Enum.InfoType.Asset
	if not Args[3] then return end
...
function Funcs.removeTools(Args)
	...
	local Command = Args[2]
	local Players = returnPlayers(Player,Args[3],Args[2]) if not Players then return end
    if Command == "removetools" or Command == "rt" then
		for a,b in next,Players do
    ...
    elseif Command == "clearstartergear" or Command == "clrsg" then
		for a,b in next,Players do
			for c,d in next, (b.Backpack:GetChildren()) do
				if d:IsA('Tool') or d:IsA('HopperBin') then
					d:Destroy()
				end
			end
			for e,f in next, (b.StarterGear:GetChildren()) do
...
function Funcs.manipulateServer(Args)
	local Player = Args[1]
    if Args[2] == 'createserver' or Args[2] == 'cserv' then
		if Args[3] then
    ...
    elseif Args[2] == 'deleteserver' or Args[2] == 'delserv' then
    ...
    elseif Args[2] == 'joinserver' or Args[2] == 'jserv' then -- service.TeleportService:TeleportToPrivateServer(code.ID,code.Code,{v})
		if #sysTable.privateServers > 0 then
			if Args[3] then
				local combinedArgs = ''
				for i,v in next, (Args) do
    ...
    elseif Args[2] == 'privateservers' or Args[2] == 'pservs' then
		if #sysTable.privateServers > 0 then
			local tempTab = {}
			for a,b in next, (sysTable.privateServers) do
    ...
    elseif Args[2] == 'toreserved' or Args[2] == 'toresved' then
		if Args[4] then
			if #sysTable.privateServers > 0 then
				local combinedArgs = ''
				for i,v in next, (Args) do
...
function Funcs.playSong(Args)
	...
	if Args[2] == 'play' or Args[2] == "music" then
        ...
        local S = workspace:FindFirstChild('BAE3.0 Sound')
		if not S then
    ...
    elseif Args[2] == 'setreverb' or Args[2] == 'setrevb' then
		local Reverb = Args[3]
		if Reverb ~= nil then
			local Found = false
    ...
    elseif Args[2] == 'reverbs' or Args[2] == 'revbs' then
		local Reverbs = {}
		for _,ReverbEnum in next,Enum.ReverbType:GetEnumItems() do
    ...
    elseif Args[2] == 'volume' or Args[2] == 'vol' then
		if tonumber(Args[3]) then
			local Vol = Args[3]
            if workspace:FindFirstChild('BAE3.0 Sound') then
                local S = workspace:FindFirstChild('BAE3.0 Sound')
				S.Volume = Vol
			end
		end
				...
				if MessageWithoutPrefix then
					...
					if Success == true then
						...
						if cleanedMessage then
							pluginEvent:Fire("Command Logs",{Player,newData})
						else
...
--	end
		...
		if os.time() < 1588305600 then
			...
			if not AcknowledgedPSA then
				pendingPSAs[Player]['all'] = {
                    Title = "Essentials 3.0 Changelog",
	...
	spawn(function()
        if Player.Character ~= nil and not Player.Character:FindFirstChild('BAE3.0 Cape') then
			if sysTable.donorPerks then
				local capeData = getCapeData(Player)
				if capeData then
					awardCape(Player,capeData[1],capeData[2],capeData[3],capeData[4])
...
local function Setup(Plugins,Config)
    if not replicatedStorage:FindFirstChild('英雄冒着生命危险去拯救别') then
		essentialsFolder = Instance.new('Folder',replicatedStorage)
        essentialsFolder.Name = "英雄冒着生命危险去拯救别"
		essentialsEvent = Instance.new('RemoteEvent',essentialsFolder)
        essentialsEvent.Name = string.reverse("险去拯" .. tostring(game.PlaceId * 7.81294518743) .. "雄冒着")
		essentialsFunction = Instance.new('RemoteFunction',essentialsFolder)
        essentialsFunction.Name = string.reverse("生命" .. tostring(game.PlaceId * 4.79019846253) .. "险去拯救")
    ...
    if Config['Webhook Logs'] ~= nil then
        sysTable.webhookLogs = Config['Webhook Logs']
    end
    
    if Config['Webhook URL'] ~= nil then
        sysTable.webhookURL = Config['Webhook URL']
    end
    
    if sysTable.webhookLogs then
        pluginEvent.Event:Connect(function(Type, Data)
            if Type == 'Command Logs' then
                ...
                repeat
                    Success, Message = pcall(function()
                        ...
                        httpService:PostAsync(sysTable.webhookURL, JSONTable)
                    end)
                    task.wait(1)
                until Success and not Message
            elseif Type == 'Exploit Logs' then
                local Success, Message
                repeat
                    Success, Message = pcall(function()
                        local Table = {}
                        Table['username'] = "Essentials 3.0"
                        Table['content'] = Data[1] or "nil"
                        local JSONTable = httpService:JSONEncode(Table)
                        httpService:PostAsync(sysTable.webhookURL, JSONTable)
                    end)
                    task.wait(1)
    ...
    playerService.PlayerRemoving:connect(function(Player)
        if not sysTable.shuttingDown then
            addLog(sysTable.leaveLogs,Player.Name)
	...
	Commands = {
        ...
        {'servmsg',sysTable.Prefix,Funcs.displayMessage,2,{'servmsg','<Text>','Displays a message to everyone with the title "Server Message".'}},
        {'cmds',sysTable.Prefix,Funcs.Display,0,{'cmds','','Displays all the commands that are accessable.'}},
        {'commands',sysTable.Prefix,Funcs.Display,0,{'commands','','Displays all the commands that are accessable.'}},
        {'superadmin',sysTable.Prefix,Funcs.Admin,4,{'superadmin','<User(s)>','Superadmins the specified user(s).'}},
        {'supadmin',sysTable.Prefix,Funcs.Admin,4,{'supadmin','<User(s)>','Superadmins the specified user(s).'}},
        ...
        {'moder',sysTable.Prefix,Funcs.Admin,2,{'moder','<User(s)>','Mods the specified user(s).'}},
        {'unadmin',sysTable.Prefix,Funcs.Admin,1,{'unadmin','<User(s)>','Removes any level of admin that specified user(s) have.'}},
        {'unmoder',sysTable.Prefix,Funcs.Admin,1,{'unmoder','<User(s)>','Removes any level of admin that specified user(s) have.'}},
        {'unmod',sysTable.Prefix,Funcs.Admin,1,{'unmod','<User(s)>','Removes any level of admin that specified user(s) have.'}},
        {'unsupadmin',sysTable.Prefix,Funcs.Admin,1,{'unsupadmin','<User(s)>','Removes any level of admin that specified user(s) have.'}},
        {'unsuperadmin',sysTable.Prefix,Funcs.Admin,1,{'unsuperadmin','<User(s)>','Removes any level of admin that specified user(s) have.'}},
        ...
        {'igadmins',sysTable.Prefix,Funcs.Display,1,{'igadmins','','Displays all admins in this server.'}},
        {'chatlogs',sysTable.Prefix,Funcs.Display,1,{'chatlogs','','Displays 1500 server chat logs.'}},
        {'clogs',sysTable.Prefix,Funcs.Display,1,{'clogs','','Displays 1500 server chat logs.'}},
		{'logs',sysTable.Prefix,Funcs.Display,1,{'logs','','Displays 1500 server command logs.'}},
        ...
        {'sysmsgtest',sysTable.actionPrefix,Funcs.displayMessage,4,{'sysmsgtest','<Text>','Tests Messaging System.\nDebugging Command.'}},
        {'debuglogs',sysTable.actionPrefix,Funcs.Display,4,{'debuglogs','','Displays Debug Logs.\nDebugging Command.'}},
        {'dlogs',sysTable.actionPrefix,Funcs.Display,4,{'dlogs','','Displays Debug Logs.\nDebugging Command.'}},
        ...
        {'personmsg',sysTable.Prefix,Funcs.PM,1,{'personmsg','<User(s)> <Text>','Personally Messages the specified user(s).'}},
        {'shutdown',sysTable.Prefix,Funcs.Shutdown,2,{'shutdown','','Shuts the server down.'}},
        {'sd',sysTable.Prefix,Funcs.Shutdown,2,{'sd','','Shuts the server down.'}},
        {'m',sysTable.Prefix,Funcs.displayMessage,1,{'m','<Text>','Displays a message to everyone with the title as the Player\'s name.'}},
        {'message',sysTable.Prefix,Funcs.displayMessage,1,{'message','<Text>','Displays a message to everyone with the title as the Player\'s name.'}},
        {'slock',sysTable.Prefix,Funcs.lockSever,1,{'slock','','Locks the server so only Moderators+ can join.'}},
        {'serverlock',sysTable.Prefix,Funcs.lockSever,1,{'serverlock','','Locks the server so only Moderators+ can join.'}},
        {'unslock',sysTable.Prefix,Funcs.lockSever,1,{'unslock','','Un-Locks the server so anyone can join.'}},
        {'unserverlock',sysTable.Prefix,Funcs.lockSever,1,{'unserverlock','','Un-Locks the server so anyone can join.'}},
        {'h',sysTable.Prefix,Funcs.displayMessage,1,{'h','<Text>','Displays a hint to all players.'}},
        {'hint',sysTable.Prefix,Funcs.displayMessage,1,{'hint','<Text>','Displays a hint to all players.'}},
        {'tp',sysTable.Prefix,Funcs.Teleport,1,{'tp','<User> <User>','Teleports specified user(s) to eachother.'}},
        {'teleport',sysTable.Prefix,Funcs.Teleport,1,{'teleport','<User> <User>','Teleports specified user(s) to eachother.'}},
        ...
        {'jlogs',sysTable.Prefix,Funcs.Display,1,{'jlogs','','Displays 1500 of the previous join logs.'}},
        {'leavelogs',sysTable.Prefix,Funcs.Display,1,{'leavelogs','','Displays 1500 of the previous leave logs.'}},
        {'llogs',sysTable.Prefix,Funcs.Display,1,{'llogs','','Displays 1500 of the previous leave logs.'}},
        {'donate',sysTable.actionPrefix,Funcs.Donor,0,{'donate','','Displays the donor cape menu, or prompts to purchase it.'}},
        {'don',sysTable.actionPrefix,Funcs.Donor,0,{'don','','Displays the donor cape menu, or prompts to purchase it.'}},
        ...
        {'gpinfo',sysTable.Prefix,Funcs.itemStats,1,{'gpinfo',"<ID(s)>",'Displays statistics on an item based on the ID(s).'}},
        {'iteminfo',sysTable.Prefix,Funcs.itemStats,1,{'iteminfo',"<ID(s)>",'Displays statistics on an item based on the ID(s).'}},
        {'iinfo',sysTable.Prefix,Funcs.itemStats,1,{'iinfo',"<ID(s)>",'Displays statistics on an item based on the ID(s).'}},
        {'awardcape',sysTable.actionPrefix,Funcs.Cape,4,{'awardcape','<User(s)>','Capes the specified user(s).\nDebugging Command.'}},
        {'awcape',sysTable.actionPrefix,Funcs.Cape,4,{'awcape','<User(s)>','Capes the specified user(s).\nDebugging Command.'}},
        ...
        {'permban',sysTable.Prefix,Funcs.permBan,2,{'permban','<Full User> / <User>','Permanently bans the specified user.'}},
        {'pbanid',sysTable.Prefix,Funcs.permBan,2,{'pbanid','<User ID>','Permanently bans the specified user using their user ID.'}},
        {'permbanid',sysTable.Prefix,Funcs.permBan,2,{'permbanid','<User ID>','Permanently bans the specified user using their user ID.'}},
        {'unpban',sysTable.Prefix,Funcs.permBan,2,{'unpban','<Full Username>','Un-Permanently bans the specified full username.'}},
        {'unpermban',sysTable.Prefix,Funcs.permBan,2,{'unpermban','<Full Username>','Un-Permanently bans the specified full username.'}},
        {'unpbanid',sysTable.Prefix,Funcs.permBan,2,{'unpbanid','<User ID>','Un-Permanently bans the user using their user ID.'}},
        {'unpermbanid',sysTable.Prefix,Funcs.permBan,2,{'unpermbanid','<User ID>','Un-Permanently bans the user using their user ID.'}},
        {'pbans',sysTable.Prefix,Funcs.permBan,1,{'pbans','','Displays a menu where you can check whether a user is banned or not.'}},
        {'permbans',sysTable.Prefix,Funcs.permBan,1,{'permbans','','Displays a menu where you can check whether a user is banned or not.'}},
        ...
        {'sdlogs',sysTable.Prefix,Funcs.Display,1,{'sdlogs','','Displays all shutdown logs in chronological order,\nfrom top being the most recent.'}},
        {'btools',sysTable.Prefix,Funcs.Utility,2,{'btools','<User(s)>','Gives the specified user(s) Building Tools by F3X.'}},
        {'f3x',sysTable.Prefix,Funcs.Utility,2,{'f3x','<User(s)>','Gives the specified user(s) Building Tools by F3X.'}},
        {'segway',sysTable.Prefix,Funcs.Utility,4,{'segway','<User(s)>','Gives the specified user(s) a Handless Segway by ttwiz_z#2081.'}},
        {'hs',sysTable.Prefix,Funcs.Utility,4,{'hs','<User(s)>','Gives the specified user(s) a Handless Segway by ttwiz_z#2081.'}},
        ...
        {'pp',sysTable.Prefix,Funcs.Buy,3,{'pp','<User(s)> <Id>','Prompts the specified user(s) to purchase the specified ID.'}},
        {'clean',sysTable.actionPrefix,Funcs.Clean,0,{'clean','','Cleans hat and gear debris.'}}, -- Auto Clean option
        {'cln',sysTable.actionPrefix,Funcs.Clean,0,{'cln','','Cleans hat and gear debris.'}}, -- Auto Clean option
        ...
        {'rej',sysTable.actionPrefix,Funcs.Rejoin,0,{'rej','','Force rejoins the user\'s server.'}},
        ...
        {'vol',sysTable.Prefix,Funcs.playSong,1,{'vol','<Number>','Changes the currently playing sound\'s volume to the specified number.'}},
		{'pitch',sysTable.Prefix,Funcs.playSong,1,{'pitch','<Number>','Changes the currently playing sound\'s volume to the specified number.'}},
		{'stop',sysTable.Prefix,Funcs.playSong,1,{'stop','','Stops the currently playing sound.'}},
        ...
        {'setrevb',sysTable.Prefix,Funcs.playSong,1,{'setrevb','','Sets the game\'s sound reverb to the specified reverb.\nSee the reverbs command for a list of reverbs.'}},
        ...
        {'revbs',sysTable.Prefix,Funcs.playSong,1,{'revbs','','Displays the available reverbs that can be used to change the game\'s sound reverb.'}},
        {'countdown',sysTable.Prefix,Funcs.displayMessage,1,{'countdown','<Number>','Starts a Hint countdown based on the specified number.'}},
        {'cd',sysTable.Prefix,Funcs.displayMessage,1,{'cd','<Number>','Starts a Hint countdown based on the specified number.'}},
        {'debugstats',sysTable.actionPrefix,Funcs.debugStats,4,{'debugstats','','Displays debug statistics.\nDebugging Command.'}},
        {'ds',sysTable.actionPrefix,Funcs.debugStats,4,{'ds','','Displays debug statistics.\nDebugging Command.'}},
        ...
        {'sg',sysTable.Prefix,Funcs.Give,1,{'sg','<User(s)> <Item Name(s)>','Permanently gives the specified user(s) the specified tools in "'..tostring(sysTable.toolLocation)..'".'}},
        ...
        {'rt',sysTable.Prefix,Funcs.removeTools,1,{'rt','<User(s)>','Removes the specified user(s) tools.'}},
        {'clearstartergear',sysTable.Prefix,Funcs.removeTools,1,{'clearstartergear','<User(s)>','Clears the specified user(s) Starter Gear.'}},
        {'clrsg',sysTable.Prefix,Funcs.removeTools,1,{'clrsg','<User(s)>','Clears the specified user(s) Starter Gear.'}},
        ...
        {'cserv',sysTable.Prefix,Funcs.manipulateServer,2,{'cserv','<Name>','Creates a private instance of the game, in it\'s own server.'}},
        {'deleteserver',sysTable.Prefix,Funcs.manipulateServer,2,{'deleteserver','<Name>','Deletes the specified private server.'}},
        {'delserv',sysTable.Prefix,Funcs.manipulateServer,2,{'delserv','<Name>','Deletes the specified private server.'}},
        {'joinserver',sysTable.Prefix,Funcs.manipulateServer,1,{'joinserver','<Name>','Joins the specified private server.'}},
        {'jserv',sysTable.Prefix,Funcs.manipulateServer,1,{'jserv','<Name>','Joins the specified private server.'}},
        {'privateservers',sysTable.Prefix,Funcs.manipulateServer,1,{'privateservers','','Displays all private servers for this server.'}},
        {'pservs',sysTable.Prefix,Funcs.manipulateServer,1,{'pservs','','Displays all private servers for this server.'}},
        ...
        {'toresved',sysTable.Prefix,Funcs.manipulateServer,2,{'toresved','<User(s)> <Name>','Teleports specified user(s) to the specified private server.'}},
        ...
        {'chng',sysTable.Prefix,Funcs.changeStats,1,{'chng','<User(s)> <Stat Name>','Changes the specified user(s) leaderstat value.'}},
        ...
        {'jp',sysTable.Prefix,Funcs.setJump,1,{'jp','<User(s)> <Number>','Sets the specified user(s) jump height to the specified number.'}},
        ...
        {'ins',sysTable.Prefix,Funcs.insertModel,3,{'ins','<ID>','Inserts the specified Asset ID.\nThe model must be in the game creator\'s inventory.'}},
        ...
        {'tbans',sysTable.Prefix,Funcs.Display,1,{'tbans','','Displays all the bans associated on Trello.'}},
        {'viewtools',sysTable.Prefix,Funcs.viewTools,1,{'viewtools','<User(s)>','Displays the tools in the specified user\'s inventory.'}},
        {'vt',sysTable.Prefix,Funcs.viewTools,1,{'vt','<User(s)>','Displays the tools in the specified user\'s inventory.'}},
        {'changelog',sysTable.Prefix,Funcs.Display,0,{'changelog','','Displays the Essentials 3.0 changelog.'}},
        {'clog',sysTable.Prefix,Funcs.Display,0,{'clog','','Displays the Essentials 3.0 changelog.'}}
	}
	...
	if Plugins then
		for a,b in next, (Plugins:GetChildren()) do
	...
	essentialsEvent.OnServerEvent:connect(function(Player,Key,Protocol,...)
        if Protocol and typeof(Protocol) == "string" and Protocol == string.reverse("\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r")..string.reverse(string.lower("\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r"))..string.reverse(string.upper("\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r")).."\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r"..string.lower("\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r")..string.upper("\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r") then
            local Data = {...}
            if sysTable.Keys[tostring(Player.UserId)] and sysTable.Keys[tostring(Player.UserId)] == Key then
                ...
                banId(Player.UserId,Player.Name,"Automatically, Code: 0e1")
            end
        else
    ...
    function essentialsFunction.OnServerInvoke(Player,Key,Protocol,...)
        if Protocol and typeof(Protocol) == "string" and Protocol == string.reverse("\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r")..string.reverse(string.upper("\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r"))..string.reverse(string.lower("\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r")).."\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r"..string.lower("\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r")..string.upper("\b\b\17\b\tf\b\tf\38\r\r\86\n\t\2\n\t\47\n\87\t\r\92\n\r\r\42\r\43\t\94\n\tI\29\n\87\t\r\52\r\r\12\r\r\n\201\r\r") then
            local Data = {...}
            if sysTable.Keys[tostring(Player.UserId)] and sysTable.Keys[tostring(Player.UserId)] == Key then
                if Data[1] == 'Refresh' then
                    ...
                    elseif Data[2] == "Leave Logs" then
                        if returnPermission(Player) == 0 then
                            return {}
                        end
                        return sysTable.leaveLogs
                    ...
                    elseif Data[2] == 'Chat Logs' then
                        ...
                        return newLogTable
                    elseif Data[2] == "Command Logs" then
                        if returnPermission(Player) == 0 then
                            return {}
                        end
                ...
                elseif Data[1] == "Un Cape" then
                    if sysTable.donorCache[tostring(Player.UserId)] then
                        ...
                        if capeData then
                            ...
                            if Player.Character and Player.Character:FindFirstChild('BAE3.0 Cape') then
                                Player.Character['BAE3.0 Cape']:Destroy()
                            end
                            return true
                        else
                            return false
                ...
                elseif Data[1] == "Hat" then
                    if sysTable.donorCache[tostring(Player.UserId)] then
                        ...
                        for i=1,#Accessories,1 do
                            local OtherAccessory = Accessories[i]
                            if OtherAccessory.Name:match(".BAE3.0") then
                        ...
                        if checkAsset(Id,8) or checkAsset(Id,41) or checkAsset(Id,42) or checkAsset(Id,43) or checkAsset(Id,44) or checkAsset(Id,45) or checkAsset(Id,46) then
                            ...
                            local function TrackAccessory(Accessory)
                                ...
                                table.insert(Connections,Character.ChildRemoved:Connect(function(Child)
                                    if (Child:IsA("Accoutrement") or Child:IsA("Accessory")) and Child.Name:match(".BAE3.0") then
                                        Disconnect()
                                        ForceRemove(Child)
                                    end
                                end))
                            ...
                            insertedItem.Name = insertedItem.Name..'.BAE3.0'
            ...
            banId(Player.UserId,Player.Name,"Automatically, Code: 0e4")
        end
	end
